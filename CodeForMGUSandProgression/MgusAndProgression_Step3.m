...
reportcell = reportcell(new_id_list,1);
...
for i = 1:length(pid)
    ...
    if MGUS_set1(i) == 1
        ...
        %%
        writetable(T_pid,['Results4_clinical_NLP_700.xlsx'],'Sheet','pid')
        writetable(TMGUS,['Results4_clinical_NLP_700.xlsx'],'Sheet','diagMGUS_time')

        if ~isempty(table2array(cell2table(MGUS_report)))
            writetable(cell2table(MGUS_report),...
                ['Results4_clinical_NLP_700.xlsx'],...
...
%     writetable(T_pid,['Results_clinical_s',num2str(s),'.xlsx'],'Sheet','pid')
    writetable(T1,['Results4_clinical_NLP_700.xlsx'],'Sheet','diagMM_time')
