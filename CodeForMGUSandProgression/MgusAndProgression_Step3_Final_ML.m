...
save_name = 'Final_061523_ML';
%% load texts as input for bag-of-words(BOW) algorithms
nminFeatures = 500;  % minimum # of appearances for this term to be a feature
removeStopWords = 1; 
doStem = 0;
...
save('NLP_700_clinical_featureVector.mat','featureVector','headers')
BOWtime = toc

%% 
load('NLP_700_clinical_featureVector.mat','featureVector','headers')
...
load('Results_clinical_700_Final080822.mat')
...
% lambda = 0.01;
% [alpha] =HSICLasso(tfidf_Matrix.',(true_train_label+1).',2,lambda);
% save('HSIC_weight.mat','alpha')
%% normalize(?)
